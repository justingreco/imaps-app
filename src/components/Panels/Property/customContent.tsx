...
export const createDurhamButton = () => {
    ...
    creator: async (e: any) => {
      ...
      const durham = await executeArcade(`var portal = Portal("https://ral.maps.arcgis.com"); 
  ...
  const deedCreator = (e: any) => {
    ...
    return (e.graphic.layer as FeatureLayer)
      ...
      .then((result) => {
        ...
        deedBtn.onclick = () => {
          window.open(
            `https://rodweb.dconc.gov/web/web/integration/search?field_BookPageID_DOT_Volume=${e.graphic.getAttribute('DEED_BOOK')}&field_BookPageID_DOT_Page=${e.graphic.getAttribute('DEED_PAGE')}`,
            "deedwindow"
          );
        };
        div.append(deedBtn);        
  ...
  export const createLinkButtons = () => {
    ...
    creator: async (e: any) => {
      ...
      const taxUrl = await executeArcade(`var portal = Portal("https://ral.maps.arcgis.com"); 
      if ($feature.CITY_DECODE == "RALEIGH - DURHAM COUNTY") { 
        var fs = FeatureSetByPortalItem(portal, "ed698c7ae43c4394b517990b1922aaba"); 
        var f= First(Filter(fs,"SITE_ADDRE = '" + $feature.SITE_ADDRESS + "'"));
  ...
  export const getDurhamPhoto = async (feature: Graphic) => {
    const photo = await executeArcade(`var portal = Portal("https://ral.maps.arcgis.com"); 
    if ($feature.CITY_DECODE == "RALEIGH - DURHAM COUNTY") { 
      var fs = FeatureSetByPortalItem(portal, "ed698c7ae43c4394b517990b1922aaba"); 
      var f= First(Filter(fs,"SITE_ADDRE = '" + $feature.SITE_ADDRESS + "'"));
