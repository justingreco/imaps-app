...
export function initializeOverview(
    ...
    const overview = new MapView({
        map: map,
        container: ref,
        navigation: {mouseWheelZoomEnabled: false, browserTouchPanEnabled: false, momentumEnabled: false }
    });
  ...
  const addWatches = (view: __esri.MapView, overview: __esri.MapView) => {
    stationaryHandle =  view.watch('stationary', () => {
      if (view.stationary) {
        overview.goTo({target: view.extent.center, zoom: view.zoom - 4},{easing: 'ease-out'});
      }
    });
    extentHandle = view.watch('extent', () =>{
        overview.graphics.removeAll();
        overview.graphics.add(new Graphic({geometry: view.extent, symbol: 
